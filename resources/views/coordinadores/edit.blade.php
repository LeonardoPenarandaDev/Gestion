<x-app-layout>
    <x-slot name="header">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
            <div style="padding: 2rem; text-align: center;">
                <h2 style="color: white; font-size: 2rem; font-weight: bold; margin: 0;">Editar Coordinador</h2>
                        ...
                        <div class="form-grid">
                            <div>
                                ...
                                <select id="municipio" class="form-select" required>
                                    ...
                                    @foreach($municipios as $m)
                                        <option value="{{ $m->municipio_codigo }}"
                                            {{ $municipioActual == $m->municipio_codigo ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="fk_id_zona" id="fk_id_zona" class="form-select" required>
                                    <option value="{{ old('fk_id_zona', $coordinador->fk_id_zona) }}">
                                        Zona {{ old('fk_id_zona', $coordinador->fk_id_zona) }}
                                    </option>
                                </select>
                            ...
                            <div>
                                ...
                                <select name="fk_id_puesto" id="fk_id_puesto" class="form-select" required>
                                    <option value="{{ old('fk_id_puesto', $coordinador->fk_id_puesto) }}">
                                        {{ $coordinador->puesto?->nombre ?? 'Puesto actual' }}
                                    </option>
                                </select>
                            ...
                            <div>
                                ...
                                <div id="mesas-info" style="padding: 0.75rem 1rem; background:#f0fdf4; border: 2px solid #86efac; border-radius:10px; color:#166534; font-weight:600; font-size:0.9rem; min-height:48px; display:flex; align-items:center;">
                                    @if($coordinador->puesto)
                                        ✓ Este coordinador cubre las {{ $coordinador->puesto->total_mesas }} mesas del puesto
                                    @else
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-grid">
                            <div>
                                ...
                                <input type="text" name="nombre" value="{{ old('nombre', $coordinador->nombre) }}"
                                       class="form-input" required maxlength="60">
                                @error('nombre') <p class="form-error">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="form-label">Número de Documento *</label>
                                <input type="text" name="documento" value="{{ old('documento', $coordinador->documento) }}"
                    ...
                    <div class="form-section" style="background: linear-gradient(135deg,rgba(79,172,254,0.05),rgba(0,242,254,0.05)); border:1px solid rgba(79,172,254,0.2);">
                        ...
                        <div class="form-grid">
                            <div>
                                ...
                                <input type="email" name="email" value="{{ old('email', $coordinador->user?->email) }}"
    ...
    const zonaActual   = "{{ $zonaActual }}";
    const puestoActual = "{{ $coordinador->fk_id_puesto }}";

    function cargarZonas(cod, zonaSelVal) {
        zonaSel.innerHTML = '<option value="">Seleccione zona</option>';
        puestoSel.innerHTML = '<option value="">Primero seleccione zona</option>';
        zonaSel.disabled = true; puestoSel.disabled = true;
        if (cod && data[cod]) {
            Object.keys(data[cod].zonas).forEach(z => {
                const o = document.createElement('option');
                o.value = z; o.textContent = `Zona ${z}`;
                if (z === zonaSelVal) o.selected = true;
                zonaSel.appendChild(o);
            });
            zonaSel.disabled = false;
            if (zonaSelVal) cargarPuestos(cod, zonaSelVal, puestoActual);
        }
    }

    function cargarPuestos(cod, zona, puestoSelVal) {
        ...
        if (cod && zona && data[cod]?.zonas[zona]) {
            data[cod].zonas[zona].forEach(p => {
                ...
                if (String(p.id) === String(puestoSelVal)) o.selected = true;
    ...
    munSel.addEventListener('change', function() { cargarZonas(this.value, ''); });
    zonaSel.addEventListener('change', function() { cargarPuestos(munSel.value, this.value, ''); });
    puestoSel.addEventListener('change', function() {
        const opt = this.options[this.selectedIndex];
        if (opt?.dataset.mesas) mesasInfo.textContent = `✓ Este coordinador cubrirá todas las ${opt.dataset.mesas} mesas del puesto`;
    });

    // Pre-cargar selección actual
    if (munSel.value) cargarZonas(munSel.value, zonaActual);
    </script>
</x-app-layout>
